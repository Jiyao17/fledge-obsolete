...
class ClientAGNEWS:
    def __init__(self) -> None:
        ...
        self.dataloader = DataLoader(train_iter, batch_size=8, shuffle=False, collate_fn=self.collate_batch)

        train_iter = AG_NEWS(split='train')
        self.num_class = len(set([label for (label, text) in train_iter]))
        self.vocab_size = len(self.vocab)
        self.emsize = 64
        self.model = TextClassificationModel(self.vocab_size, self.emsize, self.num_class).to(self.device)
